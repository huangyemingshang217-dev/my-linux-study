...
class BackgroundComposerAuthorityResolver {
  ...
  async resolve(authority, context, progress) {
    return withTimer("resolve", async () => {
      ...
      let url;
      try {
        url = await this.getCursorServerUrlWithRetry(authority, context.resolveAttempt < 3);
      } catch (error) {
...
const INVALID_ARGUMENT_RETRY_WINDOW_MS = 3e4;
const TRANSIENT_RETRY_WINDOW_MS = 3 * 6e4;
const NOT_FOUND_RETRY_WINDOW_MS = 3 * 6e4;
...
function getCursorServerUrlRetryWindowMs(error) {
  ...
  if (code === _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.InvalidArgument) {
    return INVALID_ARGUMENT_RETRY_WINDOW_MS;
  }
  if (code === _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.NotFound) {
    return NOT_FOUND_RETRY_WINDOW_MS;
  }
  if (code === _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.DeadlineExceeded || code === _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.Unavailable) {
    return TRANSIENT_RETRY_WINDOW_MS;
  }
  return void 0;
}
function isPodGoneCursorServerUrlError(error) {
  return getConnectErrorCode(error) === _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.NotFound;
}
...
function connectCodeFromName(name) {
  switch (name) {
    case "invalid_argument":
      return _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.InvalidArgument;
    case "deadline_exceeded":
      return _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.DeadlineExceeded;
    case "unavailable":
      return _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.Unavailable;
    case "not_found":
      return _connectrpc_connect__WEBPACK_IMPORTED_MODULE_0__.Code.NotFound;
    default:
      return void 0;
  }
}
